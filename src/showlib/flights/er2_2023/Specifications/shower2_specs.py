...
class SHOW_specs(object):
    ...
    def __init__(self, littrow_nm=1363.38, M=0.21910042822432865, vac = False):
        ...
        self.vfov = -self.design['height_degrees_per_pixel']*(self.ImageEdgeTop-self.ImageEdgeBottom)

        self.DetNumPixX =     self.design['Pixel_column_at_right_edge_of_grating_image']-self.design['Pixel_column_at_left_edge_of_grating_image']
        self.DetNumPixY =     self.design['Pixel_row_at_top_edge_of_atmospheric_image']-self.design['Pixel_row_at_bottom_of_atmospheric_image']
        ...
        #Center Pixel
        self.CenterPixelHorizontal = self.design['Centre_pixel_column_corresponding_to_x=0_in_the_grating_localization_plane']
        self.CenterPixelVertical   = self.design['Centre_pixel_in_vertical-spatial_dimension']

        #Dark Current Area
        self.DarkCurrentAreaBottomEdge = self.design['Pixel_row_at_bottom_edge_of_dark_current_area_below_image_area']
        self.DarkCurrentAreaTopEdge  = self.design['0_pixel_row_at_top_edge_of_dark_current_area_below_image_area']
        ...
        #Littrow angle
        self.ThetaL = np.arcsin(self.design['grating_groove_density_percm'] / (2 * self.SigmaL))  # [rad]

        #Grating dimensions
        self.GratWidthX = self.DetWidthPixX*(self.ImageEdgeRight - self.ImageEdgeLeft)*(1/self.Magnification)/np.cos(self.ThetaL)
        self.GratWidthY = self.DetWidthPixY*(self.ImageEdgeTop - self.ImageEdgeBottom)*(1/self.Magnification)/np.cos(self.ThetaL)

        self.DSigma =  (self.DetNumPixX/2)/ (4 * ((self.WidthDetX) * (1/self.Magnification)) * np.tan(self.ThetaL))# [cm^-1]
    ...
    def vac_to_air_wavels_nm(self, wavels_nm):
        ...
        n_air = 1 + 0.05792105 / (238.0185 - wavels_um ** -2) + 0.00167917 / (57.362 - wavels_um ** -2)
        air_wavels = wavels_nm / n_air
        return air_wavels

    def air_to_vac_wavels_nm(self, wavels_nm):
        wavels_um = wavels_nm * 1e-3
        n_air = 1 + 0.05792105 / (238.0185 - wavels_um ** -2) + 0.00167917 / (57.362 - wavels_um ** -2)
        vac_wavels = wavels_nm * n_air
        return vac_wavels
    ...
    def __analytic_ILS__(self, apodization = True):
        ...
        show_ils = 2 * xmax * (np.sinc(2 * xmax * (u_p - u)))  # + np.sinc(2*xmax*(u_p+u)))
    ...
    def __SHS_optical_path__(self):
        ...
        self.interferogram_x_pixel = np.arange(0, self.DetNumPixX) - self.CenterPixelHorizontal
        self.interferogram_y_pixel = np.arange(0, self.DetNumPixY) - self.CenterPixelVertical
    ...
    def __SHS_wavels__(self):
        ...
        if self.vac == True:
            self.spectrum_wavelengths = self.air_wavelength_to_vacuum_wavelength(self.spectrum_wavelengths)
            self.wav_num = np.flip(1e7/self.spectrum_wavelengths)

        self.opd_delta = delta

    def __finite_pixel_response__(self):
        self.pixel_response = np.sinc((self.wav_num - self.Littrow) * 4 * np.tan(self.ThetaL) * self.DetWidthPixX / self.Magnification)
    ...
    def __materion_filter__(self):
        ...
        for i in range(np.shape(full_trans)[1]):
            self.materion_transmittance.append( np.interp(self.wav_num, wav_filter, full_trans[:,i]))
    ...
    def __filter_correction__(self):
        ...
        self.spectral_response_wavenumbers = self.wav_num[0:247]
        self.spectral_response = np.interp(self.wav_num[0:247], abscal_data.filter_shape.data, abscal_data.wavenumbers.data)
