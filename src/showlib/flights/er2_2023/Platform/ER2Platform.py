...
class ER2Platform(object):
    ...
    def make_ER2_geometry(self, utc=['2020-07-15T15:00:00.0000000'], observer=[80, 235, 600000],
        ...
        aircraft = [aircraft_orientation[0], aircraft_orientation[1] + self.specs.boresight_pitchangle + 50*self.specs.height_degrees_per_pixel - 0.07, aircraft_orientation[2]]

        # make a platform
        platform = Platform()

        # add the measurement set
        platform.add_measurement_set(ut, ('llh', observer), ('yaw_pitch_roll', 'standard', aircraft))
        ...
        self.lines_of_sight = self.generate_SHOW_vertical_image(self.optical_axis, self.vertical_sampling)
        ...
        self.reference_tangent  = self.measurement_geometry(self.optical_axis)[0].tangent_location().altitude
        ...
        self.distance = np.linalg.norm(
            self.optical_axis_LOS.tangent_location().location - self.optical_axis_LOS.observer) / 1000

        #solar angles
        self.sza, self.saa   = self.calculate_sza_saa()


    def calc_reference_tangent(self, utc=['2020-07-15T15:00:00.0000000'], observer=[80, 235, 600000],
                                     aircraft_orientation = [0, 0, 0]):
        ...
        # add the measurement set
        platform.add_measurement_set(ut, ('llh', observer), ('yaw_pitch_roll', 'standard', aircraft_orientation))
        ...
        #Calculate the reference tangent
        reference_tangent  = self.measurement_geometry(self.optical_axis)[0].tangent_location().altitude

        return reference_tangent
    ...
    def generate_SHOW_vertical_image(self, optical_axis, vertical_sampling):
        ...
        for model_angle in model_angles:
            ...
            geo.lines_of_sight.append(sk.LineOfSight(optical_axis.mjd, optical_axis.observer, look_vector))
            los_optical_geometry.append(OpticalGeometry(look_vector=look_vector, observer=optical_axis.observer,
    ...
    def calc_line_of_sight_azimuth(self, los_optical_geometry:OpticalGeometry):
        ...
        for meas in los_optical_geometry:
            ...
            try:
                ...
                los_azimuth.append(np.arccos(np.dot(line_of_sight[0].look_vector, north)))
            except:
    ...
    def calc_distance_to_tangent(self, los_optical_geometry:OpticalGeometry):
        ...
        for meas in los_optical_geometry:
            line_of_sight = self.measurement_geometry(meas)
            try:
                observer_distance.append(np.linalg.norm(line_of_sight[0].tangent_location().location - line_of_sight[0].observer))
            except:
    ...
    def calculate_sza_saa(self):
        ...
        location = EarthLocation(lat=self.tangent_latitudes, lon=self.tangent_longitudes)
        sza = (
                90 - sun_dir.transform_to(AltAz(obstime=Time(mjd_time, format = 'mjd'), location=location)).alt.value
        )
        saa = (
                sun_dir.transform_to(AltAz(obstime=Time(mjd_time, format = 'mjd'), location=location)).az.value - 180
        )
